...
void llrfPort::SDRAM_RstEn(asynUser *pasynUser, int enable)
{
	...
	val = enable ? SDRAM_RESET : 0;
        ...
        printf("P_ramRstEn: ep=0x%04x, value=0x%04x, mask=0x%04x\n", ep, val, mask);
...
unsigned
	...
	do
	  ...
	  /*
          printf("SDRAM_Rd: okFrontPanel_ReadFromPipeOut(0x%0X,0x%02X,%d,0x%0X)= %d: ",
		 (int *)H, ep, bl_size, p,  l );
          if( l == bl_size ) printf("SUCCESS!\n");
          else               printf("%s\n", ErrorName( (ok_ErrorCode)l ) ) ;
 
          printf("sizeof(okFrontPanel_HANDLE)=%d, sizeof(int)=%d, sizeof(long)=%d, sizeof(*p)=%d\n",
		  sizeof(H), sizeof(ep), sizeof(length), sizeof(p));
	  */
          if( l != bl_size ) printf("okFrontPanel_ReadFromPipeOut error:%s\n", ErrorName( (ok_ErrorCode)l ) ) ;
...
try {
    ...
    if(function==P_load) {
        ...
        memset(bitfil0, 0, sizeof(bitfil0) );
        memset(bitfilenameRB, 0, sizeof(bitfilenameRB));
        ...
        AERR(getStringParam(P_bit_file, NELEMENTS(bitfil0), bitfil0));
	...
	printf("P_load: 1. Loaded bitfile:%s\n", bitfile);

        // HWERR2(pasynUser, okFrontPanel_LoadDefaultPLLConfiguration(H) );
        e = okFrontPanel_ConfigureFPGA(H, bitfile);

        if( e == ok_NoError ) {
            strcpy(bitfilenameRB, bitfil0);
            printf("P_load: 2. Loaded bitfile:%s - ok_NoError\n", bitfil0);
            }
        else {
            printf("P_load: 2. Loaded bitfile:%s ok_ErrorCode=%d\n", bitfile, e);
            }
        ...
        printf("P_load: 5. Loaded bitfile:%s\n", bitfile);
	...
	SDRAM_Reset(pasynUser);
        ...
        printf("function = P_ramRstEn: value=0x%04x\n", value);
	...
	printf("P_ramArm: ep=0x%04x, value=0x%04x\n", ep, val);
...
unsigned
	...
	printf("SDRAM DONE Is Triggered -\n");
...
try {
	...
	for(jj = 0; jj < DAC_COUNT; jj++ )
        ...
        scratch[2*i+0] = squash16( 32767 * FFI[i] );
        scratch[2*i+1] = squash16( 32767 * FFQ[i] );
...
//===========================================================================================================================
	...
	for(jj = 0; jj < DAC_COUNT; jj++ )
        ...
        scratch[2*i+0] = squash16( 32767 * FBI[i] );
        scratch[2*i+1] = squash16( 32767 * FBQ[i] );
...
// Nathan, March 20, 2012
    ...
    */
	...
	// EP_ADC = 0xa0, BLOCK_SIZE = 1024, ADC_LENGTH = 0x4000 = 16384
		...
		if( Log >= SCOPE_LOG_LEVEL )
			...
			*Q = zoom_v_scale[c] * (*S++ / 32767.0);
                        ...
                        if( Log >= SCOPE_LOG_LEVEL ) {
			    *pOf[c] << dec << setw(6) << i << " zoom_v_scale = " << zoom_v_scale[c] << " ";
	...
	if( FB_TrigReq ) {
            ...
            do {
                okFrontPanel_UpdateWireOuts(H);
                jj = okFrontPanel_GetWireOutValue(H, EP_WireOut_StatusBits);
                // EP_WireOut_StatusBits = 0x20, mask RampBusy = 0x02
                ii++;
            } while( ii < 100 && (jj && STATUS_FB_Busy));
